...
async function getCourses() {
    try {
        ...
        list.innerHTML = "";
        if (!courses.length) { list.innerHTML = "<li>No courses found.</li>"; return; }
        courses.forEach(c => {
            list.innerHTML += `
                <li>
...
async function loadCoursesForAssign() {
    ...
    } catch (err) { console.error("Error loading courses for assign:", err); }
}
async function createCourse() {
    ...
    } catch (err) { setMsg('createMsg', "Error creating course.", true); }
}
async function assign() {
    ...
    } catch (err) { setMsg('assignMsg', "Error assigning course.", true); }
}
...
async function submitQuiz() {
    try {
        ...
        setMsg('quizMsg', `${result.message} | Score: ${result.score}% | Attempts remaining: ${result.attempts_remaining}`, !isPass);
    } catch (err) {
...
async function verifyCert() {
    try {
        ...
        if (!cert_id) { alert("Please enter a Certificate ID"); return; }
...
async function loadDashboard() {
    try {
        ...
        courses.forEach(c => { html += `<th>${c.title}</th>`; });
        html += '</tr>';
        matrix.forEach(emp => {
            html += `<tr><td><strong>${emp.name}</strong></td><td>${emp.department}</td>`;
            courses.forEach(c => {
                const status = emp.courses[c.course_id]?.status || 'not_started';
                html += `<td><span class="status-${status}">${status.replace('_', ' ')}</span></td>`;
            });
            html += '</tr>';
        });
        html += '</table></div>';
